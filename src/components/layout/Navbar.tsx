...
const NavLink = ({ href, children, isActive }: { href: string; children: React.ReactNode; isActive: boolean }) => (
  <Link
    ...
    className={cn(
      ...
      isActive
        ? "vra-gradient-2 text-white"
        : "text-vra-purple hover:text-vra-blue hover:bg-white/10"
    )}
  >
    {children}
  </Link>
  ...
  return (
    <nav
      className={cn(
        ...
        scrolled
          ? "bg-black/20 backdrop-blur-lg border-b border-white/10 py-2"
          : "bg-transparent py-4"
      )}
    >
      <div className="container mx-auto px-4 flex items-center justify-between">
        ...
        <div className="hidden md:flex items-center space-x-1">
          ...
          <DropdownMenu>
            <DropdownMenuTrigger className="px-4 py-2 text-sm font-medium rounded-md transition-colors text-vra-purple hover:text-vra-blue hover:bg-white/10 flex items-center">
              Программы <ChevronDown className="ml-1 h-4 w-4" />
            </DropdownMenuTrigger>
            <DropdownMenuContent className="bg-black/70 backdrop-blur-md border border-vra-purple/20 text-white">
              <DropdownMenuItem>
        ...
        <button
          ...
          className="md:hidden p-2 text-vra-purple rounded-md hover:text-vra-blue"
        >
      ...
      {/* Mobile menu */}
      {isOpen && (
        <div className="md:hidden vra-glass absolute top-full left-0 right-0 p-4 border-t border-white/10 z-50">
          <div className="flex flex-col space-y-3">
            <Link
              to="/"
              className="px-4 py-2 text-vra-purple hover:text-vra-blue rounded-md"
            ...
            <div className="flex items-center justify-center gap-4 mt-2 pt-2 border-t border-white/10">
              ...
              <Link
                to="/register"
                className="py-2 px-4 vra-gradient-2 text-white rounded-md"
                onClick={() => setIsOpen(false)}
              >
                Регистрация
              </Link>
